...
UKF::UKF() {
  ...
  */
  ...
  ///* Lidar fields
  n_lidar_ = 2;
  Zsig_lidar_ = MatrixXd(n_lidar_, 2 * n_aug_ + 1);
  z_pred_lidar_ = VectorXd( n_lidar_ );
  S_lidar_ = MatrixXd( n_lidar_, n_lidar_ );
  Tc_lidar_ = MatrixXd( n_x_, n_lidar_ );
  K_lidar_ = MatrixXd( n_x_, n_lidar_ );
		...
		if( meas_pack.sensor_type_ == MeasurementPackage::RADAR )
			{
			  ...
			  double rho_dot = meas_pack.raw_measurements_[2];
...
//			// Deal with zeros
		    if(fabs(x_(0)) < EPS) x_(0) = EPS;
		    if(fabs(x_(1)) < EPS) x_(1) = EPS;
			//Set weights
...
//		if(fabs(p_x) < EPS) p_x = EPS;
//		if(fabs(p_y) < EPS) p_x = EPS;
		Zsig_lidar_(0, i) = p_x;
		Zsig_lidar_(1, i) = p_y;
	}
	...
	//Kalman gain K;
    K_lidar_ = Tc_lidar_ * S_lidar_.inverse();
...
//		if(fabs(norm) < EPS) norm = EPS;
//		if(fabs(p_x) < EPS) p_x = EPS;
//		if(fabs(p_y) < EPS) p_x = EPS;
		//TODO: finish
		Zsig_radar_(0,i) = norm;                        //r
		Zsig_radar_(1,i) = atan2(p_y,p_x);                                 //phi
		Zsig_radar_(2,i) = (p_x * v1 + p_y * v2 ) / norm;   //r_dot
	}
